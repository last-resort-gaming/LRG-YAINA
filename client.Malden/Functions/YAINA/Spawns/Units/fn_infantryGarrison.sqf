...
params ["_pos", ["_radius", [0, 30]], ["_groupCount", 1], ["_AIOB_Positioning", 2], ["_skill", 2], ["_maxFill", 4], ["_groups", []]];

if (_groups isEqualTo []) then {
    if (_groupCount isEqualTo 0) then {
        for "_x" from 0 to (_groupCount-1) do {
...
private _failed = [_pos, nil, _allUnits, _radius, _AIOB_Positioning, true, _maxFill] call DERP_fnc_AIOccupyBuilding;
