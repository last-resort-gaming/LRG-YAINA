...
private _INFTEAMS = ["HAF_InfSquad","HAF_InfSquad_Weapons","HAF_InfTeam","I_InfTeam_Light"];
...
_cqFunc = missionNamespace getVariable (selectRandom ( ["YAINA_SPAWNS_fnc", ["YAINA_SPAWNS", "CA"]] call FNC(getFunctions) ));
...
for "_x" from 0 to (3 + (random 2)) do {
    ...
    _ConquestGroup = [_randomPos, INDEPENDENT, (configfile >> "CfgGroups" >> "Indep" >> "IND_F" >> "Infantry" >> selectRandom _INFTEAMS )] call BIS_fnc_spawnGroup;
...
for "_x" from 0 to (2 + (random 2)) do {
    ...
    _unit = _grp1 createUnit ["I_Soldier_F", _randomPos, [],0,"NONE"];
    _unit assignAsDriver _vehc;
    _unit moveInDriver _vehc;
    _units pushBack _unit;


    _unit = _grp1 createUnit ["I_Soldier_AR_F", _randomPos, [],0,"NONE"];
...
[
    ...
    _missionID,
    [
        format ["An AAF Comms Station has been discovered. Clear it out!", _nearestTown select 2, text (_nearestTown select 0)],
        format ["Seize Comms Station %1 of %2", _nearestTown select 2, text (_nearestTown select 0)],
        ""
    ],
    _AOPosition,
    false,
