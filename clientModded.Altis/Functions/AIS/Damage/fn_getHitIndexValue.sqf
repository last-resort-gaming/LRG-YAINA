...
_ret = if (_hitValue isEqualType 0) then {
	...
	if (_hitValue > 10 || _hitValue < 0) exitWith {[format ["AIS ERROR: AIS_Damage_fnc_getHitIndexValue, input number is out of range: %1", _hitValue], "ErrorLog"] call YAINA_fnc_log; ["",_hitValue, 0]};
