@ECHO OFF

set MP_MISSIONS_CACHE_DIR=C:\Users\Martin\AppData\Local\Arma 3\MPMissionsCache
set SERVER_DIR=D:\Arma3Server
set SERVER_MODS="ServerMods\@Advanced Rappelling;ServerMods\@Advanced Sling Loading;ServerMods\@Advanced Towing;ServerMods\@Advanced Urban Rappelling"
set MODS="curator;kart;heli;mark;expansion;dlcbundle;jets;dlcbundle2;"
set SERVER="%SERVER_DIR%\arma3server.exe"
set PORT=2502
